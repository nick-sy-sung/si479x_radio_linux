/***************************************************************************************
                  Skyworks Inc MSS Si475x/6x/9x module
...
RET_CODE si479x_turn_audio(uint8_t mode)
{
	...
	//connect ADC
    si479x_ADC_connect(PORT_ADC0, AIN0, AUDIO_ADC_GAIN);
    si479x_ADC_connect(PORT_ADC1, AIN1, AUDIO_ADC_GAIN);
	
    //config i2s rx 1
	i2s_conf.sample_rate = FS48KS;
    i2s_conf.purpose = PURPOSE_AUDIO;
    i2s_conf.master = I2S_SLAVE;
    i2s_conf.clkfs = IN1;
    i2s_conf.data0 = DIN1;
    i2s_conf.fill = ZEROS;
    i2s_conf.slot_size = TWENTYFOUR;
    ...
    ERR_CHECKING(si479x_I2SRX1_config(&i2s_conf));
      
    //enable i2s rx 1
    ERR_CHECKING(si479x_I2S_control(RX1, PORT_ENABLE, NULL)); 
